...
def minimalWorstCaseCostForHiddenNumberGamesSumBruteForce(
    ...
    def worstCaseForRange(
        ...
        if args in memo2.keys(): return memo2[args]
        #sequence_func = lambda pivot: worstCaseForRangeWithPivot(lo, hi, pivot)
        pivot = maximumTermBruteForce(lo, hi)
...
def crossFlipsQuarterCircleMatrix(n: int) -> int:
    ...
    for row in range(n):
        i = getRowIndex(row)
        mat[i] = (1 << i) | (row_bm << getElementIndex((row, 0)))
    for col in range(n):
        i = getColumnIndex(col)
        mat[i] = (1 << i) | (col_bm << getElementIndex((0, col)))
    for i1 in range(n):
        for i2 in range(n):
            i = getElementIndex((i1, i2))
            mat[i] = (1 << i) | (1 << getRowIndex(i1)) | (1 << getColumnIndex(i2))
            d_sq = i1 * i1 + i2 * i2
    ...
    for i1 in range(m):
        ...
        while True:
            ...
            if not rng0 or j > rng0[1] + 1:
                if idx + 1 < len(seen_rngs) and rng1[0] == j + 1:
                    seen_rngs[j] = seen_rngs.popitem(idx + 1)[1]
                else: seen_rngs[j] = j
                i2 = j
            else:
                i2 = rng0[1] + 1
...
def peredurFabEfrawgMaximumExpectedBlackSheepFractionBruteForce(
    ...
    def calculateRowExpected(prev_row: list[CustomFraction]) -> list[CustomFraction]:
        ...
        def rowForRemoveWhiteBitmask(rm_white_bm: int) -> list[CustomFraction]:
            ...
            for i in range(n_tot):
                ...
                bm2 >>= 1
            #print(c_lst, d_lst)
            sol = [CustomFraction(0, 1) for _ in range(len(c_lst))]
            #print(f"c_lst = {c_lst}, d_lst = {d_lst}")
            if c_lst:
                sol[-1] = d_lst[-1]
                for i in reversed(range(len(c_lst) - 1)):
            ...
            for i2 in range(2, n_white_cutoff):
                a0 = -CustomFraction(i2, n_tot)
                c0 = -a0 - 1
                denom = 1 - a0 * c_lst[-1]
                c_lst.append(c0 / denom)
                d_lst.append(-a0 * d_lst[-1] / denom)
            a0 = -CustomFraction(n_white_cutoff, n_tot)
            c0 = -a0 - 1
        ...
        sols = []
        sol_bms = []
        prev_row2 = list(prev_row)
        prev_row2.append(CustomFraction(n_tot, 1))
        for bm in range(1 << (n_tot)):
            ...
            for i in range(len(row) - 1):
                ...
                b = bm2 & 1
                bm2 >>= 1
                #print(frac, i, prev_row2, b)
                if not b and (frac <= prev_row2[i]):
                    #print("hi")
                    break
            else:
                sols.append(row)
        ...
        if not sols:
            ...
            print("multiple solutions found for the bitmasks:")
            for bm, row in zip(sol_bms, sols):
                print(format(bm, "b").zfill(n_tot + 1), [x.numerator / x.denominator for x in row])
        return sols[0]
        """
        lo, hi = 0, n_tot - 1
        while lo < hi:
...
def peredurFabEfrawgMaximumExpectedBlackSheepFraction(
    ...
    def calculateRowExpected(prev_row: list[CustomFraction]) -> list[CustomFraction]:
        ...
        def isValidCutoff(n_white_cutoff: int) -> bool:
            ...
            c_curr = -CustomFraction(n_tot - 1, n_tot)
    ...
    for n_worthless in range(n_worthless_coins + 1):
        ...
        for worthless_pos_bm in sorted(fails[n_worthless].keys()):
            ...
            for idx0 in silver_pos_st:
                ...
                #print(f"gaps: {gaps}")
                gaps2 = tuple(gaps)
                losing_gaps_dict.setdefault(gaps2, set())
                losing_gaps_dict[gaps2].add(j0)
        ...
        for gaps in sorted(losing_gaps_dict.keys()):
            pos_set = losing_gaps_dict[gaps]
            xor_vals = [0, 0]
            for i, num in enumerate(gaps):
                xor_vals[i & 1] ^= num
            #print(f"alternate gaps xor: {xor_vals}")
            print(list(gaps), len(pos_set), pos_set, xor_vals)#, xor_vals[0] ^ xor_vals[1])
...
def maxMatrixSum(mat: list[list[int]]) -> int:
    ...
    for i1 in reversed(range(shape[0])):
        prev_bms = dict(curr_bms)
        for bm, score in prev_bms.items():
            bm2 = ((1 << shape[1]) - 1) ^ bm
            while bm2:
                lo = bm2 & (-bm2)
                i2 = lo.bit_length() - 1
                bm3 = bm | lo
                curr_bms[bm3] = max(curr_bms.get(bm3, -float("inf")), score + mat[i1][i2])
                bm2 ^= lo
    return max(curr_bms.values())

def getProjectEulerProblem345Matrix() -> list[list[int]]:
    return [
...
def maxMatrixSumForProjectEulerProblem345Matrix() -> int:
    """
    Solution to Project Euler #345
    """
