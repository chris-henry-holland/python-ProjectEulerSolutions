...
def alexandrianIntegerGenerator() -> Generator[int, None, None]:
    ...
    so a loop over this generator must include a
...
# Problem 223
def barelyAcuteIntegerSidedTrianglesAscendingPerimeterGenerator(
    max_perim: Optional[int]=None,
) -> Generator[Tuple[int, int, int], None, None]:
    """
    Generator iterating over all barely acute integer sided triangles
    in increasing order of perimeter, for which (if specified) the
    perimeter does not exceed max_perim.

    A barely acute integer sided triangle is a triangle whose side
    ...
    Args:
        ...
        max_perim (int or None): If specified as an integer, the
                ...
                acute integer sided triangles yielded, otherwise
    ...
    Yields:
    3-tuple of integers (int) representing a barely acute integer
...
def countBarelyAcuteIntegerSidedTrianglesUpToMaxPerimeter(
    max_perimeter: int=25 * 10 ** 6,
) -> int:
    """
    Solution to Project Euler #223

    Calculates the number of barely acute integer sided triangles
    ...
    Args:
        ...
        max_perim (int): Integer giving the inclusive upper bound
                on the perimeter of barely acute integer sided
                triangles yielded included in the total.
            Default: 25 * 10 ** 6
    
    Returns:
    Integer (int) giving the number of barely acute integer sided
...
# Problem 238
def infiniteStringTourDigitSumStartSum(
    n_max: int=2 * 10 ** 15,
    s_0: int=14025256,
    s_mod: int=20300713,
    base: int=10,
    ...
    For the infinite string of digits generated by
    concatenating the terms in a Blum Blum Shub
    pseudo-random number generator expressed in the chosen
    base in order, calculates the sum of the 1-indexed
    starting locations in this string of the first
    occurrence of a contiguous substring whose digit sum
    is equal to each integer between 1 and n_max inclusive.

    The terms in the Blum Blum Shub sequence, s_i for a
    given seed value (s_0) and modulus (s_mod) is
    ...
    Args:
        ...
        n_max (int): Strictly positive integer giving the largest
                number whose representation in the chosen base
                is to be found in the infinite digit string.
            Default: 2 * 10 ** 15
        s_0 (int): Strictly positive integer that is strictly less
                than s_mod giving the seed value for this Blum Blum
                Shub sequence.
            Default: 14025256
        s_mod (int): Integer strictly greater than 1 giving the
                modulus for this Blum Blum Shub sequence.
            Default: 20300713
        base (int): Integer strictly greater than 1 giving the
                base in which the terms in the Blum Blum Shub
                pseudo-random number generator should be expressed
                when the infinite digit string is generated.
            Default: 10
            
    Returns:
    Integer (int) giving the sum of the 1-indexed starting locations
    in the infinite digit string generated as described by the
    Blum Blum Shub pseudo-random number generator with seed value
    s_0 and modulus s_mod of the first occurrence of a contiguous
    substring whose digit sum is equal to each integer between 1 and
    n_max inclusive.
